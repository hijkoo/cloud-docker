...
if [ "${PHP_EXTENSIONS}" != "" ]; then
    apk --update add --no-cache --virtual .build-deps autoconf gcc g++ libtool make curl-dev gettext-dev linux-headers libffi-dev libffi-dev openssl-dev git
fi
...
installExtensionFromTgz()
{
    ...
    if test $extensionName = 'swoole'; then
        ( cd ${extensionName} && phpize && ./configure --enable-openssl --enable-http2 --enable-mysqlnd && make ${MC} && make install )
    else
